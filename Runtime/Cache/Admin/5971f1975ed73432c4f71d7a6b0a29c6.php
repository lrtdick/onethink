		...
		<table>
			...
			<tbody>
					<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$channel): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><?php echo ($channel["intro"]); ?></td>
                        <td><?php echo ($channel["room_number"]); ?></td>
						<td><?php echo ($channel["status"]); ?></td>
						<td>
							<a title="编辑" href="<?php echo U('edit?id='.$channel['id'].'&pid='.$pid);?>">编辑</a>
							<a href="<?php echo U('setStatus?ids='.$channel['id'].'&status=1');?>" class="ajax-get"><?php echo (show_status_op($channel["status"])); ?></a>
							<a class="confirm ajax-get" title="删除" href="<?php echo U('setStatus?ids='.$channel['id'].'&status=0');?>">删除</a>
						</td>
